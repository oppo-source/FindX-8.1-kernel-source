...
#ifdef VENDOR_EDIT
	/*Added by Zhengrong.Zhang@Cam.Drv, 20180421, for [ois calibration debug]*/
	//check sum
	if (1) {
        	uint32_t sum_check = 0;
        	rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0xF008, &sum_check,
        		CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_DWORD);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "read sum_check fail");
                } else {
			CAM_INFO(CAM_OIS, "sum_check = 0x%x", sum_check);
		}
	}
#endif
