...
#ifdef VENDOR_EDIT
/*xing.xiong@BSP.Kernel.Driver, 2018/04/19, Modify for LRA*/
...
#ifdef VENDOR_EDIT
/*xing.xiong@BSP.Bootloader.Bootflow, 2018/03/20, Add for min vibrator time*/
...
#else
		if (chip->play_mode != HAP_BUFFER || (chip->play_mode == HAP_BUFFER && time_ms > HAP_BUFFER_MIN_CYCLE))
#endif
...
#ifdef VENDOR_EDIT
/*xing.xiong@BSP.Kernel.Driver, 2018/07/10, Add debug log for release version*/
	pr_info("active %d\n", val);
#endif
...
static int qpnp_haptics_config(struct hap_chip *chip)
{
	...
	if (chip->act_type == HAP_LRA) {
		...
		pr_debug("Drive period code max limit 0x%x min limit 0x%x\n",
			chip->drive_period_code_max_limit,
			chip->drive_period_code_min_limit);
	}
