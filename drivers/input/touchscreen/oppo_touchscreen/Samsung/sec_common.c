...
static ssize_t proc_corner_control_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    ssize_t ret = 0;
    int ver_x = 0, ver_y = 0, hor_x = 0, hor_y = 0;
    char page[PAGESIZE];
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
    struct sec_proc_operations *sec_ops = (struct sec_proc_operations *)ts->private_data;

    if (!ts || !sec_ops->get_corner_range)
        return count;

    sec_ops->get_corner_range(ts->chip_data, &ver_x, &ver_y, &hor_x, &hor_y);
    sprintf(page, "%d %d %d %d\n", ver_x, ver_y, hor_x, hor_y);
    ret = simple_read_from_buffer(user_buf, count, ppos, page, strlen(page));
...
{
    char buf[128] = {0};
    int ret, panel_dir, corner_bit, corner_x, corner_y;
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
    struct sec_proc_operations *sec_ops = (struct sec_proc_operations *)ts->private_data;

    if (!ts || !sec_ops->set_corner_range)
        return count;
    ...
    sscanf(buf, "%d,%d,%d,%d", &panel_dir, &corner_bit, &corner_x, &corner_y);
    mutex_lock(&ts->mutex);
    TPD_INFO("%s: panel_dir(%d), corner_bit(%d), corner_x(%d), corner_y(%d)\n", __func__, panel_dir, corner_bit, corner_x, corner_y);
    sec_ops->set_corner_range(ts->chip_data, panel_dir, corner_bit, corner_x, corner_y);
    if (ts->is_suspended == 0) {
        ret = ts->ts_ops->mode_switch(ts->chip_data, MODE_EDGE, ts->limit_edge);
        if (ret < 0) {
            TPD_INFO("%s, Touchpanel operate mode switch failed\n", __func__);
...
int sec_create_proc(struct touchpanel_data *ts, struct sec_proc_operations *sec_ops)
{
    ...
    prEntry_tmp = proc_create_data("curved_range", 0664, ts->prEntry_tp, &proc_curved_control_ops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }

    prEntry_tmp = proc_create_data("grip_handle", 0664, ts->prEntry_tp, &proc_corner_control_ops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }
