...
static void sec_set_corner_range(void *chip_data, int dir, int corner_bit, int x, int y)
{
    struct chip_data_s6sy761 *chip_info = (struct chip_data_s6sy761 *)chip_data;

    if (dir) {      //means in portrait
        chip_info->grip_area.ver_grip_x = x;
        chip_info->grip_area.ver_grip_y = y;
    } else {
        if (corner_bit & (1 << LANDSCAPE_ELIM_BIT)) {
            chip_info->grip_area.hor_elim_x = x;
            chip_info->grip_area.hor_elim_y = y;
        } else {
            chip_info->grip_area.hor_grip_x = x;
            chip_info->grip_area.hor_grip_y = y;
        }
    }
}

static void sec_get_corner_range(void *chip_data, int* p_ver_x, int* p_ver_y, int* p_hor_x, int* p_hor_y)
{
    struct chip_data_s6sy761 *chip_info = (struct chip_data_s6sy761 *)chip_data;

    *p_ver_x = chip_info->grip_area.ver_grip_x;
    *p_ver_y = chip_info->grip_area.ver_grip_y;
    *p_hor_x = chip_info->grip_area.hor_grip_x;
    *p_hor_y = chip_info->grip_area.hor_grip_y;
}
...
static struct sec_proc_operations sec_proc_ops = {
    ...
    .set_corner_range   = sec_set_corner_range,
    .get_corner_range   = sec_get_corner_range,
};
