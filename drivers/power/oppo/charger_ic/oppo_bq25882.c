...
/**********************************************************
  ...
  *********************************************************/
/*
static int bq25882_read_interface (int RegNum, int *val, int MASK, int SHIFT)
{
    int bq25882_reg = 0;
    int ret = 0;
   ...
   //chg_err("--------------------------------------------------\n");
    ...
    ret = bq25882_read_reg(RegNum, &bq25882_reg);
	
   //chg_err(" Reg[%x]=0x%x\n", RegNum, bq25882_reg);
	
    bq25882_reg &= (MASK << SHIFT);
    *val = (bq25882_reg >> SHIFT);
...
//	if(RegNum == REG03_BQ25882_ADDRESS)
...
#if 0
int bq25882_reg_config_interface (int RegNum, int val)
{   
    int ret = 0;
	
	mutex_lock(&bq25882_i2c_access);
    ret = __bq25882_write_reg(RegNum, val);
	mutex_unlock(&bq25882_i2c_access);
...
int bq25882_input_current_limit_write(int value)
{
	int rc = 0,i = 0,j = 0;
	int chg_vol = 0;
	struct chip_bq25882 *chip = charger_ic;
    ...
    for(j = 1; j <= i; j++) {
		switch (j) {
			case 1:
				rc = bq25882_config_interface(REG03_BQ25882_ADDRESS, REG03_BQ25882_INPUT_CURRENT_LIMIT_1000MA, REG03_BQ25882_INPUT_CURRENT_LIMIT_MASK);
			break;
			case 2:
				rc = bq25882_config_interface(REG03_BQ25882_ADDRESS, REG03_BQ25882_INPUT_CURRENT_LIMIT_1200MA, REG03_BQ25882_INPUT_CURRENT_LIMIT_MASK);
			break;
			case 3:
				rc = bq25882_config_interface(REG03_BQ25882_ADDRESS, REG03_BQ25882_INPUT_CURRENT_LIMIT_1500MA, REG03_BQ25882_INPUT_CURRENT_LIMIT_MASK);
			break;
			case 4:
				rc = bq25882_config_interface(REG03_BQ25882_ADDRESS, REG03_BQ25882_INPUT_CURRENT_LIMIT_1700MA, REG03_BQ25882_INPUT_CURRENT_LIMIT_MASK);
			break;
			case 5:
				rc = bq25882_config_interface(REG03_BQ25882_ADDRESS, REG03_BQ25882_INPUT_CURRENT_LIMIT_2000MA, REG03_BQ25882_INPUT_CURRENT_LIMIT_MASK);
			break;
			default:
				break;
		}     
...
aicl_end:
	...
	if((j >= 0) && (j <= ARRAY_SIZE(bq25882_usbin_input_current_limit) - 1))
		aicl_result = bq25882_usbin_input_current_limit[j];		
	chg_debug( "usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_end\n", chg_vol, j, bq25882_usbin_input_current_limit[j], chip->sw_aicl_point);
	switch (j) {
		case 0:
			rc = bq25882_config_interface(REG03_BQ25882_ADDRESS, REG03_BQ25882_INPUT_CURRENT_LIMIT_500MA, REG03_BQ25882_INPUT_CURRENT_LIMIT_MASK);
		break;
		case 1:
			rc = bq25882_config_interface(REG03_BQ25882_ADDRESS, REG03_BQ25882_INPUT_CURRENT_LIMIT_1000MA, REG03_BQ25882_INPUT_CURRENT_LIMIT_MASK);
		break;
...
#if 0
static bool bq25882_check_charger_suspend_enable(void)
{
	int rc = 0;
	int reg_val = 0;
	...
	rc = bq25882_read_reg(REG01_BQ25882_ADDRESS, &reg_val);
	if (rc) {
		chg_err("Couldn't read REG01_BQ25882_ADDRESS rc = %d\n", rc);
		return false;
	}

	charger_suspend_enable = ((reg_val & REG01_BQ25882_EN_HIZ_MASK) == REG01_BQ25882_EN_HIZ_ENABLE) ? true : false;
...
int bq25882_adc_en_enable(bool enable)
{
	...
	if (enable == false) {
		rc = bq25882_config_interface(REG15_BQ25882_ADDRESS,
			0x0, 0xFF);//REG15_BQ25882_ADC_EN_DISABLE, REG15_BQ25882_ADC_EN_MASK);
		if (rc < 0) {
			chg_err("Couldn't bq25882_adc_en_disable  rc = %d\n", rc);
		}
	} else {
		rc = bq25882_config_interface(REG15_BQ25882_ADDRESS,
			0x80, 0xFF);//REG15_BQ25882_ADC_EN_ENABLE, REG15_BQ25882_ADC_EN_MASK);
		if (rc < 0) {
...
int bq25882_ibus_adc_dis_enable(void)
{
    ...
    rc = bq25882_config_interface(REG16_BQ25882_ADDRESS, 
		0x00, 0xFF);//REG16_BQ25882_IBUS_ADC_DIS_ENABLE, REG16_BQ25882_IBUS_ADC_DIS_MASK);
    if (rc < 0) {
...
int bq25882_ichg_adc_dis_enable(void)
{
    ...
    rc = bq25882_config_interface(REG16_BQ25882_ADDRESS, 
		0x00, 0xFF);//REG16_BQ25882_ICHG_ADC_DIS_ENABLE, REG16_BQ25882_ICHG_ADC_DIS_MASK);
    if (rc < 0) {
