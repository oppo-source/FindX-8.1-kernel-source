...
/*xing.xiong@BSP.Kernel.Driver, 2018/05/07, Modify for disable TX*/
extern bool g_disable_tx;
static bool disable_rx(void)
{
	return true;
}
...
#ifdef VENDOR_EDIT
/* Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/04/26, Add for enale/disable HStty1 pin for smart mic */
...
#ifdef VENDOR_EDIT
/*xing.xiong@BSP.Kernel.Driver, 2018/05/08, Add for disable serial input*/
...
static int msm_geni_serial_port_setup(struct uart_port *uport)
{
	...
	if (!uart_console(uport)) {
		...
		if (!msm_port->rx_buf) {
		#ifndef VENDOR_EDIT
		/*xing.xiong@BSP.Kernel.Statbility, 2018/05/16, Modify for memory free issue*/
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/04/10, sjc Add for console */
...
static int msm_geni_serial_probe(struct platform_device *pdev)
{
	...
	#ifdef VENDOR_EDIT
	/*xing.xiong@BSP.Kernel.Driver, 2018/06/07, Modify for pull up gpio4&5 in ftm mode*/
	if (drv->cons && (get_boot_mode() == MSM_BOOT_MODE__FACTORY)) {
