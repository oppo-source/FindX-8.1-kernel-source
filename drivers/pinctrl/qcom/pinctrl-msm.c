...
#ifdef VENDOR_EDIT
//PengNan@BSP.Power.Basic 2018/06/14 add for resume irq.
				  ...
				  unsigned gpio)
{
	...
	u32 ctl_reg;
#ifdef VENDOR_EDIT
/*xing.xiong@BSP.Bootloader.Bootflow, 2018/04/03, Add for get gpio status*/
	int in_value;
	int out_value;
	u32 io_reg;
#endif
...
static void msm_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	#ifdef VENDOR_EDIT
	/*xing.xiong@BSP.Kernel.Debug, 2018/05/11, Add for cat gpio*/
...
static void msm_gpio_dirconn_handler(struct irq_desc *desc)
{
	...
	chained_irq_enter(chip, desc);
	#ifdef VENDOR_EDIT
	//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/05/28, Add log for smart mic log
	if(irqd->hwirq == 80){
		pr_info("Receive GPIO80 irq, irqd->hwirq:%lu, irqd->irq:%u\n", irqd->hwirq, irqd->irq);
	}
	#endif /* VENDOR_EDIT */
