...
#ifdef VENDOR_EDIT
/*jie.hu@PSW.MM.Display.Lcd.Stability,2018/6/9,Bug 1408488, for checking dsi pll unlock issue,add a panic here*/
#include "sde_dbg.h"
#endif
...
static int dsi_link_clk_prepare(struct dsi_link_clks *l_clks)
{
	...
	if (rc) {
		...
		#ifdef VENDOR_EDIT
		/*jie.hu@PSW.MM.Display.Lcd.Stability,2018/6/16,Bug 1408488, for checking dsi pll unlock issue,add a panic here*/
		SDE_DBG_DUMP("all", "dsi_dbg_bus");
		panic("add for dsi pll unlock");
		#endif
		goto byte_clk_err;
	}
