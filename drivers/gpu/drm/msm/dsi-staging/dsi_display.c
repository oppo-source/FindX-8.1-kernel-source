...
#ifdef VENDOR_EDIT
/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-04-13,add for drm notifier for display connect*/
#include <linux/msm_drm_notify.h>
#include <linux/notifier.h>
extern int msm_drm_notifier_call_chain(unsigned long val, void *v);
/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-04-21,add for solve sau issue*/
extern int lcd_closebl_flag;

/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-04-23,add for avoid print so much log*/
bool shoud_print_backlight = false;
int set_backlight_count =0;
DEFINE_MUTEX(backlight_print_lock);

/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-04-30,add for polling err_fg when esd check,our err_fg is gpio 11*/
int gpio_err_fg = 11;
#endif
...
int dsi_display_set_backlight(void *display, u32 bl_lvl)
{
	...
	//#ifdef VENDOR_EDIT
	/*jie.hu@PSW.MM.Display.LCD.Stability,2018/3/22,add for silence reboot*/
	/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-04-21,add for solve sau issue*/
	if(lcd_closebl_flag) {
	...
	//#ifdef VENDOR_EDIT
	/*jie.hu@PSW.MM.Display.LCD.Stability,2018/3/17,add for save select panel and give different feature*/
	if(shoud_print_backlight == true) {
		pr_err("%s: level=%d\n", __func__, (int)bl_temp);
		set_backlight_count ++;
		if(set_backlight_count >= 3) {
			mutex_lock(&backlight_print_lock);
			shoud_print_backlight = false;
			mutex_unlock(&backlight_print_lock);
			set_backlight_count = 0;
		}
	} else if(bl_temp == 0) {
		pr_err("%s: level=%d\n", __func__, (int)bl_temp);
	}
		...
		struct dsi_panel *panel)
{
	...
	/* acquire panel_lock to make sure no commands are in progress */
	//#ifdef VENDOR_EDIT
	/*Jie.Hu@PSW.MM.DisplayService,  2018/07/06,  Add for solve backlight and esd check crash issue*/
...
//#ifdef VENDOR_EDIT
/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-03-26,add to mark power states*/
/**
*For kernel space :
*SDE_MODE_DPMS_ON	0
*SDE_MODE_DPMS_LP1	1
...
#ifndef VENDOR_EDIT
//*jie.hu@PSW.MM.Display.LCD.Stability,2018/4/13,implement our set power mode here/
				  ...
				  loff_t *ppos)
{
	...
	#ifndef VENDOR_EDIT
	/*jie.hu@PSW.MM.Display.Lcd.Stability,2018/6/5,for solve coverity issue*/
...
#ifdef VENDOR_EDIT
/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-04-30,add for polling err_fg when esd check*/
	if (gpio_is_valid(gpio_err_fg)) {
		rc = gpio_direction_input(gpio_err_fg);
		if (rc)
			pr_err("unable to set gpio_err_fg to input rc=%d\n", rc);
	}
#endif
...
#ifndef VENDOR_EDIT
/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-05-05,add a key log for display probe*/
...
#ifdef VENDOR_EDIT
//*jie.hu@PSW.MM.Display.LCD.Stability,2018/3/23,when continous splash enabled, we should set power mode to OPPO_DISPLAY_POWER_ON here*/
		set_oppo_display_power_status(OPPO_DISPLAY_POWER_ON);
/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-04-23,add for avoid print so much log*/
		mutex_lock(&backlight_print_lock);
		shoud_print_backlight = true;
		mutex_unlock(&backlight_print_lock);
#endif
		return 0;
	}
...
int dsi_display_disable(struct dsi_display *display)
{
	int rc = 0;
#ifdef VENDOR_EDIT
/*jie.hu@PSW.MM.Display.LCD.Stability,2018/4/13, add a notify for when disable display*/
	int blank;
	struct msm_drm_notifier notifier_data;
#endif
...
#ifdef VENDOR_EDIT
/*Jie.Hu@PSW.MM.Display.Lcd.Stability, 2018-07-24,add a delay here for ir issue*/
	if(OPPO_DISPLAY_AOD_SCENE == get_oppo_display_scene()) {
		usleep_range(160* 1000, 160* 1000);
	}
#endif
	SDE_EVT32(SDE_EVTLOG_FUNC_EXIT);
	return rc;
...
#ifdef VENDOR_EDIT
//*jie.hu@PSW.MM.Display.LCD.Stability,2018/3/20,add for support aod,hbm,seed*/
