...
/*xing.xiong@BSP.Kernel.Debug, 2018/06/29, Add for print alarm time and rtc time*/
...
/*xing.xiong@BSP.Kernel.Debug, 2018/06/29, Add for print alarm time and rtc time*/
	rc = qpnp_read_wrapper(rtc_dd, value,
				...
				NUM_8_BIT_RTC_REGS);
	if (rc) {
		dev_err(&pdev->dev, "Read from Alarm reg failed\n");
	} else {
		secs = TO_SECS(value);
		...
		if (!rc) {
			dev_info(&pdev->dev, "Alarm after - h:r:s=%d:%d:%d, d/m/y=%d/%d/%d\n",
				time.tm_hour, time.tm_min,
				time.tm_sec, time.tm_mday,
				time.tm_mon, time.tm_year);
		}
	}

	rc = qpnp_rtc_read_time(&pdev->dev, &time);
	if (!rc) {
		dev_info(&pdev->dev, "RTC time - h:r:s=%d:%d:%d, d/m/y=%d/%d/%d\n",
				time.tm_hour, time.tm_min,
				time.tm_sec, time.tm_mday,
				time.tm_mon, time.tm_year);
	}
