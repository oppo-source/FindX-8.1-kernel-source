...
EXPORT_SYMBOL(afe_dsm_setget_params);
...
int afe_dsm_ramp_dn_cfg(uint8_t *payload, int delay_in_ms)
{
	...
	*(params + 4) = 0x03000064; // mute time
	#ifndef VENDOR_EDIT
	/*Le.Li@PSW.MM.AudioDriver.Codec, 2018/08/17,
	 * Modify  to solve 5s no sound after start mic to rec
	 */
	*(params + 5) = 5000; // 5s mute time will make sure silence output till PA software shutdown.
	#else /* VENDOR_EDIT */
	*(params + 5) = 200; // 200ms mute time will make sure silence output till PA software shutdown.
	#endif /* VENDOR_EDIT */
...
static struct cal_block_data *afe_find_cal(int cal_index, int port_id)
{
	...
	#ifndef VENDOR_EDIT
	//Zhaoan.Xu@PSW.MM.AudioDriver.Platform, 2018/05/22, Modify for remove err log
