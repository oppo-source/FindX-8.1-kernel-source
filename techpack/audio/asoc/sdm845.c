...
#ifdef VENDOR_EDIT
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/05/31, Add for smart mic
...
#ifdef VENDOR_EDIT
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec.1263116, 2018/03/13, Add for smart mic
			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		#ifndef VENDOR_EDIT
		//Zhaoan.Xu@PSW.MM.AudioDriver.Codec.1263116, 2018/03/13, Modify for smart mic
		return msm_snd_enable_codec_ext_tx_clk(codec, 1, true);
		#else /* VENDOR_EDIT */
		if (knowles_mic_status) {
			dmic1_highz_enable(1);
			iacore_set_spl_pdm_mode_sts(1);
			dmic1_highz_enable(0);
			rc = msm_snd_enable_codec_ext_tx_clk(codec, 1, true);
		} else {
			return msm_snd_enable_codec_ext_tx_clk(codec, 1, true);
		}
		break;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (knowles_mic_status) {
			dmic1_highz_enable(1);
			rc = msm_snd_enable_codec_ext_tx_clk(codec, 0, true);
			//add for wait pdm clock close then to open uart
			msleep(5);
			iacore_set_spl_pdm_mode_sts(0);
		} else {
			return msm_snd_enable_codec_ext_tx_clk(codec, 0, true);
		}
		break;
...
#define S(X, Y) ((WCD_MBHC_CAL_PLUG_TYPE_PTR(tavil_wcd_cal)->X) = (Y))
	#ifndef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/07/18,
...
#ifndef VENDOR_EDIT
/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1357982, 2018/04/24, modify for headset button det */
	btn_high[0] = 75;
	btn_high[1] = 150;
	btn_high[2] = 237;
	btn_high[3] = 500;
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (++mi2s_intf_conf[index].ref_cnt == 1) {
		...
		#ifndef VENDOR_EDIT
		//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/14, Modify for remove error log
		if (index == QUAT_MI2S) {
		#else /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/*Zhaoan.Xu@PSW.MM.AudioDriver.SmartPA, 2018/04/03, Add for Audience*/
static struct snd_soc_dai_link audience_fe_dai[] = {
	{/* hw:x,40 */
...
#ifdef VENDOR_EDIT
/*Zhaoan.Xu@PSW.MM.AudioDriver.Codec.1263116, 2018/01/08, Modify for smart mic*/
...
#ifndef VENDOR_EDIT
/*Zhaoan.Xu@PSW.MM.AudioDriver.SmartPA, 2018/01/08, Modify for Max98928*/
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "tavil_codec")) {
				...
				for (i = 0; i < ARRAY_SIZE(msm_mi2s_be_dai_links); i++) {
					...
					if (temp_link->id == MSM_BACKEND_DAI_QUATERNARY_MI2S_RX) {
						...
						memcpy(temp_link, &audience_be_dai_links[0],
							...
							pr_info("%s: register  audience_be_dai_links TX %s\n",
								__func__, product_name);
					}
				}
			}
		...
		/*Zhaoan.Xu@PSW.MM.AudioDriver.Codec.1263116, 2018/04/14, Modify for smart mic*/
		memcpy(msm_tavil_snd_card_dai_links + total_links,
			audience_fe_dai,
			sizeof(audience_fe_dai));
		total_links += ARRAY_SIZE(audience_fe_dai);
		#endif /* VENDOR_EDIT */
