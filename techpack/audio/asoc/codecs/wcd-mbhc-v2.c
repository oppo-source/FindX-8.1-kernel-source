...
#ifdef VENDOR_EDIT
/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1272920, 2018/04/02, Add for oppo project */
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		#ifndef VENDOR_EDIT
		/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2017/03/03,
		 * Delete for headphone detect.
		 */
		...
		#ifndef VENDOR_EDIT
		/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/03/22, Modify for log*/
...
extern void oppo_set_fulldump_ready(void);
#endif
				   ...
				   enum wcd_mbhc_plug_type plug_type)
{
	...
	#ifndef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/03/22, Modify for necessary log */
...
#define pr_debug pr_info
	...
	#ifdef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/03/22, Add for headset*/
...
int wcd_mbhc_get_button_mask(struct wcd_mbhc *mbhc)
{
	...
	#ifdef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.Codec, 2018/02/22,
...
#define pr_debug pr_info
	...
	if (mbhc->selfstick_dev.state & SELFSTICK_BTN_PLUG_IN) {
		if (mbhc->selfstick_dev.state & SELFSTICK_BTN_LONG_PRESS) {
			extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_LONG_PRESS, false);
		}
		else {
			extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_LONG_PRESS, true);
		}
	}
	#endif
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			...
			if (mbhc->in_swch_irq_handler) {
				...
				//Le.Li@PSW.MM.AudioDriver.Feature.SelfstickDetect, 2018/04/30, add for selfstick detect
				if (mbhc->selfstick_dev.state & SELFSTICK_BTN_PLUG_IN) {
					pr_err("%s: It is selfstick short button press, buttons_pressed = 0x%x\n", __func__, mbhc->buttons_pressed);
					if ((mbhc->selfstick_dev.state & SELFSTICK_BTN_SHORT_PRESS)  &&
					(mbhc->selfstick_dev.state & SELFSTICK_BTN_PLUG_IN)) {
						pr_err("%s: set SELFSTICK_BTN_SHORT_PRESS false\n", __func__);
						extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_SHORT_PRESS, false);
						pr_err("%s: after clean short button press, state = 0x%x\n", __func__, mbhc->selfstick_dev.state);
					}
					else {
						pr_err("%s: set SELFSTICK_BTN_SHORT_PRESS true\n", __func__);
						extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_SHORT_PRESS, true);
						pr_err("%s: after short button press, state = 0x%x\n", __func__, mbhc->selfstick_dev.state);
					}
					msleep(80);
				}
				#endif /* VENDOR_EDIT */
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	#ifndef VENDOR_EDIT
	//Le.Li@PSW.MM.AudioDriver.HeadsetDet.1421705, 2018/06/14, Modify the Debounce to 32ms
...
extern int fsa4480_enable_audio_switch(void);
extern int fsa4480_disable_audio_switch(void);
int wcd_cancel_usbc_abnormal_adapter_dwork(struct wcd_mbhc *mbhc);
/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/08/09,
 * Add to be compatible with as6313.
 */
					     ...
					     bool active)
{
	...
	if (active) {
		 ...
		 */
		mbhc->mbhc_cb->lock_sleep(mbhc, true);
		if (is_project(OPPO_17107) && (HW_VERSION__19 == get_PCB_Version())){
			mbhc->jack_state = 1;
			as6313_CTIA();
		} else {
			mbhc->jack_state = fsa4480_enable_audio_switch();
		}
		mbhc->mbhc_cb->lock_sleep(mbhc, false);

		/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/05/12, Add for abnormal adapter check */
		if (mbhc->jack_state == 7 || mbhc->jack_state == 6) {
			/* fsa4480 check it maybe adapter, set extn true */
			pr_err("%s: jack_state = %d, dwork to set selfstick or recheck!\n", __func__, mbhc->jack_state);

			mbhc->mbhc_cb->lock_sleep(mbhc, true);
			if (schedule_delayed_work(&mbhc->usbc_abnormal_adapter_check_dwork, msecs_to_jiffies(1000)) == 0){
				mbhc->mbhc_cb->lock_sleep(mbhc, false);
				pr_err("%s: start schedule work failed!\n", __func__);
			}
		} else {
			mbhc->invalid_jack_cound = 0;
		}
		#endif /* VENDOR_EDIT */
		...
		if (mbhc->jack_state == 7 || mbhc->jack_state == 6) {
			mbhc->jack_state = 0;
			wcd_cancel_usbc_abnormal_adapter_dwork(mbhc);
			pr_err("%s: cancel_delayed_work usbc_abnormal_adapter_check_dwork\n", __func__);
		}
		mbhc->invalid_jack_cound = 0;
		#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1272920, 2018/03/10,
   Add for using Fsa4480 to check headphones type on T0 board */
extern int fsa4480_enable_switch_check(void);
extern int fsa4480_check_pin_state(void);
EXPORT_SYMBOL(wcd_mbhc_mech_plug_detect_irq);
#endif /* VENDOR_EDIT */

#ifdef VENDOR_EDIT
/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1272920, 2018/03/10, add new function for micbias*/
static void wcd_mbhc_usbc_sync_micbias4_selfstick(struct wcd_mbhc *mbhc) {

	pr_err("%s: mbhc->usbc_mode = %d\n", __func__, mbhc->usbc_mode);

	if (mbhc->usbc_mode == POWER_SUPPLY_TYPEC_NONE) {

		#ifdef VENDOR_EDIT
		//Le.Li@PSW.MM.AudioDriver.Feature.SelfstickDetect, 2018/04/30, add for selfstick detect
		extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_PLUG_IN, false);
		extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_SHORT_PRESS, false);
		extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_LONG_PRESS, false);
		#endif /* VENDOR_EDIT */
...
#define FSA4480_CHECK_INVALID_JACK_COUND_MAX 3

static void wcd_mbhc_usbc_abnormal_adapter_dwork_fn(struct work_struct *work)
{
	struct delayed_work *dwork;
	struct wcd_mbhc *mbhc;

	dwork = to_delayed_work(work);
	mbhc = container_of(dwork, struct wcd_mbhc, usbc_abnormal_adapter_check_dwork);

	pr_err("%s: mbhc->jack_state = %d, invalid_jack_cound = %d\n", __func__, mbhc->jack_state, mbhc->invalid_jack_cound);

	if (6 == mbhc->jack_state) {
		extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_PLUG_IN, true);
		mbhc->invalid_jack_cound = 0;
	}
	else if (7 == mbhc->jack_state && mbhc->invalid_jack_cound < FSA4480_CHECK_INVALID_JACK_COUND_MAX) {
		mbhc->invalid_jack_cound++;
		wcd_mbhc_usb_c_analog_setup_gpios(mbhc,
				mbhc->usbc_mode != POWER_SUPPLY_TYPEC_NONE);
		wcd_mbhc_usbc_sync_micbias4_selfstick(mbhc);
	}
...
int wcd_cancel_usbc_abnormal_adapter_dwork(struct wcd_mbhc *mbhc) {
	int ret;
	ret = cancel_delayed_work_sync(&mbhc->usbc_abnormal_adapter_check_dwork);
	if (ret) {
		mbhc->mbhc_cb->lock_sleep(mbhc, false);
	}
	return ret;
...
static void wcd_mbhc_usbc_analog_work_fn(struct work_struct *work)
{
	 ...
	 */
	if(mbhc->usbc_mode == POWER_SUPPLY_TYPEC_NONE) {
		pr_err("%s: Type C have been plug out cancel delayed work\n", __func__);
		cancel_delayed_work_sync(&mbhc->hp_detect_work);
	}
	#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/05/12, Add for code optimizing */
	wcd_mbhc_usbc_sync_micbias4_selfstick(mbhc);
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1362115, 2018/04/02, Add for user verision init */
static void wcd_mbhc_usbc_analog_init_work_fn(struct work_struct *work)
{
	struct delayed_work *dwork;
	struct wcd_mbhc *mbhc;
	struct usbc_ana_audio_config *config;
	...
	dwork = to_delayed_work(work);
	mbhc = container_of(dwork, struct wcd_mbhc, usbc_analog_init_dwork);
	config = &mbhc->mbhc_cfg->usbc_analog_cfg;

	pr_err("%s: mbhc->usbc_mode = %d\n", __func__, mbhc->usbc_mode);

	/* disabling GPIOs */
	if ((POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER == mbhc->usbc_mode) && (MBHC_PLUG_TYPE_NONE == mbhc->current_plug)) {
		if (config->usbc_en1_gpio_p)
			msm_cdc_pinctrl_select_sleep_state(
				config->usbc_en1_gpio_p);
		if (config->usbc_force_gpio_p)
...
static int wcd_mbhc_usb_c_analog_init(struct wcd_mbhc *mbhc)
{
	...
	#ifdef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1362115, 2018/04/02, Add for user verision init */
	INIT_DELAYED_WORK(&mbhc->usbc_analog_init_dwork, wcd_mbhc_usbc_analog_init_work_fn);
	#endif /* VENDOR_EDIT */
	#ifdef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.HeadsetDet, 2018/05/12, Add for abnormal adapter check */
	INIT_DELAYED_WORK(&mbhc->usbc_abnormal_adapter_check_dwork, wcd_mbhc_usbc_abnormal_adapter_dwork_fn);
	#endif /* VENDOR_EDIT */
	...
	#ifdef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1362115, 2018/04/02, Add for user verision init */
	schedule_delayed_work(&mbhc->usbc_analog_init_dwork,
...
static int wcd_mbhc_usb_c_analog_deinit(struct wcd_mbhc *mbhc)
{
	#ifdef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.HeadsetDet.1362115, 2018/04/02, Add for user verision init */
	cancel_delayed_work_sync(&mbhc->usbc_analog_init_dwork);
	#endif /* VENDOR_EDIT */
		      ...
		      bool impedance_det_en)
{
	...
	#ifdef VENDOR_EDIT
	//Le.Li@PSW.MM.AudioDriver.Feature.HeadsetDet, 2018/05/24, add for fsa4480 res det
	mbhc->jack_state = 0;
	mbhc->invalid_jack_cound = 0;

	//Le.Li@PSW.MM.AudioDriver.Feature.SelfstickDetect, 2018/04/30, add for selfstick detect
	{
		pr_err("%s: register selfstick\n", __func__);

		memset(&mbhc->selfstick_dev, 0x0, sizeof(mbhc->selfstick_dev));
		mbhc->selfstick_dev.supported_cable = wcd_ext_mbhc_supported_cable;
		mbhc->selfstick_dev.dev.parent = mbhc->codec->dev;

		mbhc->selfstick_dev.state= false;

		ret = extcon_dev_register(&mbhc->selfstick_dev);
		if (ret) {
			pr_err("%s: failed to register selfstick_dev, ret=%d\n", __func__, ret);
		}
		mbhc->selfstick_dev.name = "selfstick";

		pr_err("%s: register all selfstick success!\n", __func__);
	}
	#endif /* VENDOR_EDIT */
...
void wcd_mbhc_deinit(struct wcd_mbhc *mbhc)
{
	...
	#ifdef VENDOR_EDIT
	//Le.Li@PSW.MM.AudioDriver.Feature.SelfstickDetect, 2018/04/30, add for selfstick detect
	extcon_dev_unregister(&mbhc->selfstick_dev);
	#endif /* VENDOR_EDIT */
}
