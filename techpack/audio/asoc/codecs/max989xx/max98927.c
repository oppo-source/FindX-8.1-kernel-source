...
#ifdef VENDOR_EDIT
/*Zhaoan.Xu@PSW.MM.AudioDriver.FTM, 2018/04/03, Add for get spk revsion for maxim98927*/
static bool is_maxim_series(int rev){
	bool ret = false;
	if((rev & 0xff) == 0x42){
		ret = true;
	}
...
retry:
	ret = regmap_read(max98927->regmap[MAX98927L], MAX98927_REV_ID, &rev);
	if (ret < 0 || !is_maxim_series(rev)) {
		...
		const struct i2c_device_id *id)
{
	...
	pr_info("enter  max98927_i2c_probe %s\n", __func__);
	if (!max98927) {
