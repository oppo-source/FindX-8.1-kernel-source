		...
		struct task_struct *p, u32 old_load)
{
	...
	#ifdef VENDOR_EDIT
	//Haitao.Zhou@BSP.Kenrel.Stability, 2018/05/27, Modify for fixing scheduler_tick  kernel NULL pointer dereference
	if (!grp || !p->grp)
		return 0;
	#else
	if (!grp)
...
void walt_irq_work(struct irq_work *irq_work)
{
	...
	for_each_sched_cluster(cluster)
		...
		{
			/*
			 * since walt_irq_work will evalute cpu util for all cluster cpus,
			 * there is not necessary to calculate next freq and submit freq change each time.
			 * only the last time evaluate can reflect the real util for current cluster cpus.
			 * set SCHED_CPUFREQ_WALT_NOT_READY to bypass calculattion of next freq and submit freq change.
			 *
			 * if next_cpu is online and is valid, then this cpu can bypass.
			 */
			flag = orig_flag;
			next_cpu = cpumask_next(cpu, &cluster->cpus);
			if (next_cpu != nr_cpu_ids && cpu_online(next_cpu))
				flag |= SCHED_CPUFREQ_WALT_NOT_READY;
#endif
